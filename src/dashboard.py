    ...
    try:
        ...
        if user_role in ["Admin", "Doctor/Nurse"]:
            ...
            if choice == "Overview":
                ...
                age_histogram = px.histogram(patient_df, x='age', nbins=20, title="Age Distribution",
                ...
                if search_query:
                    ...
                    if not df.empty:
                        st.dataframe(df)
                        medication_id = st.selectbox("Select Medication ID to Update", df["Medication ID"].unique())
                        selected_row = df[df["Medication ID"] == medication_id].iloc[0]
                        medication_name = st.text_input("Medication Name", value=selected_row["Medication Name"])
                        medication_type = st.text_input("Medication Type", value=selected_row["Medication Type"])
                        date_prescribed = st.date_input("Date Prescribed", value=pd.to_datetime(selected_row["Date Prescribed"]))
                        treatment_used = st.text_input("Treatment Used", value=selected_row["Treatment Used"])
                        side_effect = st.text_input("Side Effect", value=selected_row["Side Effect"])

                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("Update Medication"):
                                update_medication(medication_id, medication_name, medication_type, date_prescribed, treatment_used, side_effect)
                                st.success(f"Medication {medication_id} updated successfully!")
                        with col2:
                ...
                if search_query:
                    ...
                    if not df.empty:
                        st.dataframe(df)
                        appointment_id = st.selectbox("Select Appointment ID to Update", df["Appointment ID"].unique())
                        selected_row = df[df["Appointment ID"] == appointment_id].iloc[0]
                        appointment_date = st.date_input("Appointment Date", value=pd.to_datetime(selected_row["Appointment Date"]))
                        appointment_time = st.time_input("Appointment Time", value=pd.to_datetime(selected_row["Appointment Time"]).time())
                        treatment_type = st.text_input("Treatment Type", value=selected_row["Treatment Type"])
                        appointment_type = st.text_input("Appointment Type", value=selected_row["Appointment Type"])

                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("Update Appointment"):
                                update_appointment(appointment_id, appointment_date, appointment_time, treatment_type, appointment_type)
                                st.success(f"Appointment {appointment_id} updated successfully!")
                        with col2:
            ...
            reminder_days = st.sidebar.radio("Send reminder for upcoming appointments in:", (3, 1))
            if st.sidebar.button(f"Send Reminder for Appointments in {reminder_days} Day(s)"):
                appointments_df = get_upcoming_appointments(reminder_days)
                if not appointments_df.empty:
                    st.write(f"Sending reminders for appointments in the next {reminder_days} day(s):")
        ...
        telegram_icon = "https://upload.wikimedia.org/wikipedia/commons/8/83/Telegram_2019_Logo.svg"
        ...
        st.sidebar.markdown("<h3 style='color: #fff;'>Contact Us</h3>", unsafe_allow_html=True)
